...
<form action="${pageContext.request.contextPath}/profile/results" method="get">
    ...
    <option
            <c:if test="${requestScope.sort.equals('score')}">selected</c:if> value="score"><fmt:message key="msg.score"/>
    </option>
    <option
            <c:if test="${requestScope.sort.equals('block')}">selected</c:if> value="topic"><fmt:message key="msg.topic"/>
    </option>
</select>
