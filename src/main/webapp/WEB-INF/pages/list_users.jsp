...
<div class="container">
    ...
    <form action="${pageContext.request.contextPath}/profile/users" method="get">
        ...
        <option
                <c:if test="${requestScope.sort.equals('date')}">selected</c:if> value="date"><fmt:message key="msg.date"/>
        </option>
        <option
                <c:if test="${requestScope.sort.equals('score')}">selected</c:if> value="score"><fmt:message key="msg.date"/>
        </option>
        <option
                <c:if test="${requestScope.sort.equals('block')}">selected</c:if> value="block"><fmt:message key="msg.block"/>
        </option>
    </select>
        ...
        <div id="table">
            ...
            <c:forEach items="${requestScope.users}" var="user">
                    <div class="row">
                        ...
                        <span class="cell primary" data-label="Vehicle">${user.email}</span>
  ...
  cursor: pointer;">
                        ...
                        <c:if test="${user.block==1}"><fmt:message key="msg.block-user"/></c:if>
                          <c:if test="${user.block==0}"><fmt:message key="msg.unblock-user"/></c:if></button>
